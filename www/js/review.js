...
function verdict(now, prev, state) {
  ...
  if (dSessions <= -3) return { level: 'warn', text: `${Math.abs(dSessions)} fewer sessions than last week. Consistency is the whole mechanism here — drop the daily target to one rather than missing days entirely.` };
  ...
  mount.innerHTML = `
    <div class="screen">
      ...
      ${state.pe.sessions.length ? `<section class="card">
        ...
        <p class="fineprint">Target is two hours of stretching a day — 14 hours across a full week.</p>
      </section>` : ''}
